...
def _infer_var_data_type_shape_(grad_var_name, block):
    ...
    if block.desc.has_var_recursive(cpt.to_bytes(fwd_name)):
        ...
        grad_var.set_dtype(
            fwd_var.dtype())  # grad_var 与 fwd_var 保持一致的 dtype 和 shape
...
def _addup_repetitive_outputs_(op_descs, block_idx):
    ...
    renamed_vars = collections.defaultdict(
        list)  # key：var_name,  value：rename 后的 new_name
    renamed_var_start_idx = collections.defaultdict(
        list
    )  # key：var_name,  val: start_idx ---- idx 记录的是反向图中 op 的序号(第一次 rename 的 op idx)
    ...
    for idx, op_desc in enumerate(op_descs):  # 2. 遍历 反向图
        ...
        for var_name in op_desc.input_arg_names():  # 3. 遍历 反向 op 的输入
            if "@GRAD" not in var_name:
                continue
            if len(
                    renamed_vars[var_name]
            ) > 1:  # 4. 检查 var_name 对应的 grad_var 有多少个，即多少个 grad_var 的梯度需要聚合，进行聚合操作
        ...
        for param_idx, param_name in enumerate(op_desc.output_names(
            ...
            for arg_idx, var_name in enumerate(arg_names):
                ...
                if len(
                        renamed_vars[var_name]
                ) == 0:  # 6. 判断当前 遇到的 var_name 是否记录过，如果没有，加入 renamed_vars 记录起来，并且记录下反向图中的 op 序号
                    ...
                    if len(renamed_vars[var_name]) == 1:
                        ...
                        var_rename_count[
                            var_name] += 1  # 更名次数+1            # 7.2  在 var_rename_count 中记录 var_name 更名次数
                        # rename original var_name
                        renamed_vars[var_name][
                            0] = new_name  # 修改原始的名字     # 7.3 renamed_vars 里面更新 名字
                        ...
                        _rename_arg_(
                            op_descs,
                            var_name,
                            new_name,  # 7.4 在 op_descs 里面修改对应的 var_name （需要遍历操作）
                            renamed_var_start_idx[var_name],
                            idx)
                        _rename_arg_(
                            pending_sum_ops, var_name,
                            new_name)  # 7.5 在 后接的 sum op 里面修改对应的 var_name

                        for p in op_desc.output_names(
                        )[:param_idx]:  # 8. 遍历当前反向 op 的  前 param_idx 个输出
                    ...
                    op_desc.set_output(param_name,
                                       arg_names)  # 9.3 op_desc 里面更名
                    renamed_vars[var_name].append(
                        new_name)  # 9.4 renamed_vars 里记录下来，新的命名
                    # 9.5 record the latest device
                    var_device[var_name] = op_device
    # 到这，已经遍历完反向图
    for var_name, inputs in six.iteritems(
            renamed_vars
    ):  # 10. 遍历所记录的 renamed_vars 字典, [var_name]:[new_name1, new_name2,...]
        if len(renamed_vars[var_name]) > 1:
            if len(renamed_vars[var_name]) > _MAX_ADD_NUM_:
                _accumulate_gradients_by_sum_op_(  # 11. 将 op 输出的 那些 需要做梯度聚合的 var_name 进行聚合，即对应的 [new_name1, new_name2,...] 做聚合
                    var_name,
                    renamed_vars,
                    pending_sum_ops,  # 实际是在后面加上 sum op 
                    len(op_descs),
                    var_device[var_name])
            else:
...
def _append_backward_ops_(
            ...
            for op_desc in grad_op_desc:
                ...
                for name in op_desc.output_arg_names():
                    ...
                    if block.desc.find_var(name.encode("ascii")):
                        ...
                        )  # 'grad/' * grad_order + name
                        op_desc._rename_output(name, new_name)  # 设置 新 name
                        rename_var_map[
                            name] = new_name  # 旧名字 -> 新名字，记录 旧：新 名字的关系
        ...
        if input_grad_names_set is not None:  # 6.2 非第一次调 gradient 接口
            ...
            for op_desc in grad_op_desc:  # 6.2.1 遍历反向 op 
                ...
                if _some_in_set_(input_grad_names, input_grad_names_set):
                    ...
                    is_append_grad = True
                    for name in op_desc.output_arg_names(
                    ):  # 又将反向 op 的输出，放入 input_grad_names_set 里面
                        input_grad_names_set.add(name)
            if is_append_grad:
                grad_to_var.update(op_grad_to_var)
        else:  # 6.1 第一次调 gradient，直接进这个分支
            grad_op_descs.extend(
                grad_op_desc)  # 处理完一个 反向op, 加入 grad_op_descs list 里
    ...
    not_need_ops = _find_not_need_ops(
        grad_op_descs, ops,
        input_grad_names_set)  # input_grad_names_set 辅助找到一些不需要的反向 op
...
def _append_backward_vars_(block, start_op_idx, grad_to_var, grad_info_map):
    ...
    for op_idx in range(start_op_idx, block.desc.op_size()):
        ...
        if op_desc.has_attr("sub_block"):
            ...
            _append_backward_vars_(sub_block, 0, grad_to_var,
                                   grad_info_map)  # 递归调用
        ...
        if not outputs:
            ...
            if grad_var_ins:
                existing_grad_var_ins = [
                    ...
                    var in  # has_var_recursive 意思是递归地去找是否有这个 var 的存在 --- 命名有点怪
                    parent_op_vars
                ]
        ...
        for arg in op_desc.output_arg_names():  # 遍历反向 op 的所有输出
            if arg in new_vars:
                _infer_var_data_type_shape_(
                    arg, block)  # 将 grad_var 的 dtype,shape 与 fwd_var 保持一致
...
def _rename_grad_(block, start_op_idx, grad_to_var, target_grad_map):
    ...
    for op_idx in range(start_op_idx, block.desc.op_size()):  # 遍历反向图所有 op
        ...
        for name in op_desc.input_arg_names():  # 遍历反向 op 的所有输入
            if name in var_map:  # 
                op_desc._rename_input(
                    name, var_map[name])  # 修改反向op的 输入命名, 用 用户提供的那个替代
    ...
    for g, ng in six.iteritems(
            var_map):  # key: grad_name/grad_var, val: non_grad_name/var
        if g in grad_to_var:
    ...
    for i, op in reversed(list(enumerate(op_path))):  # 反序 遍历 op_path
        ...
        if not op.has_attr("sub_block"):
            for out_var in op.desc.output_arg_names():  # 遍历当前 op 的所有输出
                if out_var not in output_names and out_var not in op.desc.input_arg_names(
                        ...
                        out_var].stop_gradient:  # 后边 stop_gradient 似乎有点奇怪
        ...
        for name in op.desc.input_arg_names():  # 遍历当前 op 的所有输入, name
            if name not in no_grad_set:  # 如果 name 不在 no_grad_set
                output_names.add(
                    name)  # 则将其 name 加入 output_names 里面， 遍历的下一个 op 需要用到
...
def _find_op_path_(block,
    ...
    if inputs:  # 用户提供的 inputs 不为空
        for i, op in enumerate(block.ops):  # 前向 遍历当前 block 里面的所有 op
            if _some_in_set_(
                    ...
                    ),  #  op.desc.input_arg_names() 当前op所有输入。   input_names 是用户提供的一个输入
                    input_names) and core.has_non_empty_grad_op_maker(
                        op.type):  # 并且当前 op 的反向 op 不是 empty_grad_op_maker
                for name in op.desc.output_arg_names():  # 遍历op的输出names 
                    if name not in no_grad_set:  # 过滤一下
                        input_names.add(
                            name
                        )  # 为何将 op.desc.output_arg_names() 的 name 放入到 input_names 里面？？？
                ...
                relevant_op_flags[
                    i] = False  # 未使用的 op，flags 改为 False, 即表明这些 op 不需要创建反向 op

    # 将 block.ops 的列表反转
    for i, op in reversed(list(enumerate(
            block.ops))):  # 反序 遍历 当前 block 里面的所有 op
        if op.has_attr("sub_block"):
        ...
        if _some_in_set_(
            ...
            for name in op.desc.input_arg_names():  # 遍历当前 op 的输入 name
                if name not in no_grad_set:  # 过滤一下
                    output_names.add(
                        name
                    )  # 为何将 op.desc.input_arg_names() 的 name 放入 output_names 里面？
    ...
    op_path = [
        block.ops[i] for i in range(len(block.ops))
        if relevant_op_flags[i]  # relevant_op_flags 记录相关的前向 op
    ]

    if inputs:
        for op in op_path:
            for name in op.desc.input_arg_names():
                if name not in input_names and block.vars[
                        name].stop_gradient:  #  op_path 里，所有op的输入 name, 如果不在 input_names 并且 stop_gradient 的话，加入 no_grad_set
...
def calc_gradient(targets, inputs, target_gradients=None, no_grad_set=None):
    ...
    if no_grad_set is None:
        ...
        no_grad_set = _get_no_grad_set_name(
            copy.copy(no_grad_set))  # 6.1 做了些预处理
    ...
    for i, grad in enumerate(target_gradients):
        ...
        if grad is None:  # 为 None, fill_constant 1.0
            ...
            block.desc.append_op().copy_from(
                op_desc)  # block.desc 里面增加这个 op_desc
            input_grad_names_set.add(
                grad_name
            )  #  input_grad_names_set 什么鬼命名？input 是只对反向图来说，是输入, 这个放着 out@GRAD
        else:  # 非 None 情况
            # 先预处理一些东西
            ...
            target_grad_map[_append_grad_suffix_(
                target.name)] = grad.name  # grad.name ==> target_gradients 的名字
    ...
    block_no_grad_set = set(map(_strip_grad_suffix_,
                                no_grad_dict[0]))  # 这里又去掉 @GRAD 尾缀
    ...
    no_grad_dict[0].update(
        list(map(_append_grad_suffix_, block_no_grad_set)))  # 又加上 @GRAD 尾缀
    ...
    _rename_grad_(
        block, fwd_op_num, grad_to_var, target_grad_map
    )  # target_grad，如果用户没传入，则为空map；如果用户传入，则不为空，target_grad_map[out@GRAD]= 传入的 target_gradients.name 
